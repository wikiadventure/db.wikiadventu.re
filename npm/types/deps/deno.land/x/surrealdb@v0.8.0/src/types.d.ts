...
export interface Connection {
    ...
    merge?: <T extends Record<string, unknown>, U extends Record<string, unknown> = T>(thing: string, data?: MergeData<T, U>) => Promise<(MergeData<T, U> & {
        id: string;
    })[]>;
    patch?: (thing: string, data?: Patch[]) => Promise<Patch[]>;
    delete?: <T extends Record<string, unknown>>(thing: string) => Promise<(T & {
        id: string;
    })[]>;
}
...
export type HTTPConnectionOptions<TFetcher = typeof dntShim.fetch> = ConnectionOptions & {
    fetch?: TFetcher;
};
export type MergeData<T extends Record<string, unknown>, U extends Record<string, unknown> = T> = T & U;
